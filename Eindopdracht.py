...
if pages== 'Map' or pages == 'Economic change' or pages == 'Comparison disasters':
    ...
    quantiles = rampen_df.groupby(['Disaster Group', 'Disaster Subgroup', 'Disaster Type',
        'Disaster Subtype'])['Intensity'].quantile([0.25, 0.75]).reset_index()
    quantiles = quantiles.pivot(index=['Disaster Group', 'Disaster Subgroup', 'Disaster Type',
       'Disaster Subtype'], columns = 'level_4', values='Intensity').reset_index()
    test = rampen_df.merge(quantiles, left_on=['Disaster Group', 'Disaster Subgroup', 'Disaster Type',
        'Disaster Subtype'], right_on=['Disaster Group', 'Disaster Subgroup', 'Disaster Type',
        'Disaster Subtype'], how='left')
    test.columns = ['Year','ISO','Country','Disaster Group','Disaster Subgroup','Disaster Type',
                    'Disaster Subtype','Total Deaths','Total Affected new',
                    'Intensity','Jaar 0','Jaar 1','Jaar 2','Jaar 3',"0.25",'0.75']
    test['Category']=0
